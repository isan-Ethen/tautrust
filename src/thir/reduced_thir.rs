...
impl<'a, 'tcx> ThirReducer<'a, 'tcx> {
  ...
  pub fn reduce(&mut self) {
    ...
    let new_body = self.reduce_expr();
    self.reduced_thir.get_mut().set_body(new_body);
  }

  pub fn get_reduced_thir(&mut self) -> RThir<'tcx> { self.reduced_thir.steal() }

  pub fn reduce_expr(&self) -> Option<RExpr<'tcx>> {
    let expr = ExprId::from_usize(self.thir.exprs.len() - 1);
    let Expr { ty: _, temp_lifetime: _, span, kind: _ } = &self.thir[expr];
    Some(RExpr::new(RExprKind::new(None), *span))
  }
}
...
//   pub fn new(thir: &'a Thir<'tcx>) -> Self { Self { thir, fmt: String::new() } }
...
//     let Block { targeted_by_break: _, span: _, region_scope: _, stmts, expr, safety_mode: _ } =
...
//         print_indented!(self, format!("remainder_scope: {:?}", remainder_scope), depth_lvl + 2);
...
//     let Expr { ty: _, temp_lifetime: _, span, kind } = &self.thir[expr];
...
//
//     match expr_kind {
//       Scope { region_scope: _, value, lint_level: _ } => {
...
//     print_indented!(self, format!("variant_index: {:?}", adt_expr.variant_index), depth_lvl + 1);
